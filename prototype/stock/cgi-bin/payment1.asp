...
<%
	...
	shipaccount = SQLEncode(Request("shipaccount")) 
	accountname = SQLEncode(Request("accountname")) 


	' Perform IDC style DefaultParameter processing.
	If IsEmpty(Request.form("Shipping")) Or Request.form("Shipping") = "" Then
		shipcost = 0
		shiptype="none selected"
		shipservice="none selected"
		Else
			STR=Request.form("Shipping")
			Dim getshipping
			getshipping=Split(STR,";")
			shiptype=getshipping(0)
			shipservice=getshipping(1)
			shipcost=getshipping(2)
	End If
	...
	SQLStmt = SQLStmt & "SET ShipType='" & shiptype & "', "
	SQLStmt = SQLStmt & "ShipService='" & shipservice & "', "
	SQLStmt = SQLStmt & "ShipCost='" & shipcost & "', "
	SQLStmt = SQLStmt & "shipaccount='" & shipaccount & "', "
	SQLStmt = SQLStmt & "accountname='" & accountname & "' "
...
<head>
<meta HTTP-EQUIV="Refresh" CONTENT="0; URL=payment2.asp">
<title>Retrieve Confirmation</title>
...
<p><!--webbot bot="PurpleText" PREVIEW="This asp file saves the information from the shipping page." --></p>
