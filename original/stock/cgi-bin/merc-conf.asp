<%
	'initial variables that the shopping cart customer may need to set
	'this is the email address you wish to have the email sent FROM
		mymail = Session("myemail")			
	'this is the name of the file and the absolute path of where the header file is
		myheader = Session("mytext")		
	'specify the subject of your email here
		mysubject = Session("confsubject")			
...
%>
<%
	'	read in a file that the customer provides as the header for this email
	Dim fs, a, themessage
	Dim theItem(100)
	Dim Description(100)
	Dim Quantity(100)
	Set fs = CreateObject("Scripting.FileSystemObject")
	Set a = fs.OpenTextFile(myheader)
	Do While a.AtEndOfStream <> True
    themessage = themessage & a.ReadLine & chr(13)
Loop
a.Close

	' create an email CDONTS object
	Set objMail = CreateObject("CDONTS.NewMail")

CurrentRecord = 0 
Do While CheckRS(RS)				'loop through database fields

	toaddress=RS("email")				'email to address	
	...
	theItem(CurrentRecord)= RS("Item")
	Description(CurrentRecord) = RS("Description")
	Quantity(CurrentRecord) = RS("Quantity") 
...
Loop
	...
	themessage = themessage & "-----------------------------------------" & chr(13)
	themessage = themessage & "The details of an order are as follows-" & chr(13)
	themessage = themessage & chr(13)
	themessage = themessage & "The Order # is:  "  & order & chr(13)
	themessage = themessage & "The order total is:  "  & grandtotal & chr(13)
	themessage = themessage & "The order must be shipped to:  "  & shipfname & " " & shiplname & chr(13)
	themessage = themessage & "at:  "  & shipaddress & "; " & shipcity & ", " & shipstate & ", " & shipzip & chr(13)
	themessage = themessage & "The order must ship via:  "  & shiptype & chr(13)
	themessage = themessage & "and needs to be billed to:  " & fname & " " & lname & chr(13)

For x=0 to (CurrentRecord - 1)
	themessage = themessage & "  -Send " & Quantity(x) & " of Item # " & theItem(x) & "; " & Description(x) & chr(13)
Next
	themessage = themessage & "-----------------------------------------" & chr(13)
	...
	mailres=objMail.Send()		'send email
%>
...
<p align="left"><!--webbot bot="PurpleText" PREVIEW="' Copyright (c) 1997-2000 ComCity Corporation.  All Rights Reserved
...
<p align="left"><!--webbot bot="PurpleText" PREVIEW="This asp file sends your customer a receipt by email.

THIS ASP REQUIRES THAT YOU INSTALL THE SMTP SERVICE THAT COMES WITH IIS4.0
MICROSOFT HAS NOT CURRENTLY PROVIDED THE TECHNOLOGY TO RUN THIS ON THE PWS.

To Configure:
Edit the global.asa file and change
               'this is your email address
		mymail = &quot;no-one@comcity.com&quot;			
	'this is the name of the file and the absolute path of where the header file is
		mytext = &quot;d:\web\support-test\mikeb20\cgi-bin\email.txt&quot;
	'specify the subject of your email here
		receiptsubject = &quot;ComCity SalesCart Order&quot;	" --></p>
